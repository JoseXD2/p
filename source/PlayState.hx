...
class PlayState extends MusicBeatState
{
	...
	public function startVideoCornered(name:String):Void {
		#if VIDEOS_ALLOWED
		var foundFile:Bool = false;
		var fileName:String = #if MODS_ALLOWED Paths.modFolders('videos/' + name + '.' + Paths.VIDEO_EXT); #else ''; #end
		#if sys
	...
	public function startDialogue(dialogueFile:DialogueFile, ?song:String = null):Void
	{
		...
		if(dialogueFile.dialogue.length > 0) {
			...
			FlxG.log.warn('Your dialogue file is badly formatted!'); // YOU are badly formatted
			if(endingSong) {
				endSong();
			} else {
				startCountdown();
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			#if desktop
			if (startTimer != null && startTimer.finished && !skippedIntro)
			{
		...
		if (startingSong)
			{
				if (startedCountdown)
				{
					if (FlxG.keys.justPressed.SPACE && !skippedIntro)
						{
							skippedIntro = true;
							pacSound.stop();
							remove(skippedText);
			...
			case 'Hey!':
				...
				if(value != 0) {
					if(dad.curCharacter.startsWith('gf')) { //Tutorial GF is actually Dad! The GF is an imposter!! ding ding ding ding ding ding ding, dindinding, end my suffering
															// Whoever wrote this comment ^, I know you live next to your neighbors - broster
	...
	// Help I got lost and i have no idea where I am
	//                                     - broster
